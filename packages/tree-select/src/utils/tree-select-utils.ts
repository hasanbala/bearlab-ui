...
export const isLeaf = (node: TreeSelectOption): boolean =>
  !node.children || node.children.length === 0;

export const buildNodeIndex = (treeData: TreeSelectOption[]): NodeIndex => {
  const byValue = new Map<NodeValue, TreeSelectOption>();
...
export const collectEnabledLeaves = (
  node: TreeSelectOption
): TreeSelectOption[] => {
...
): Array<{ node: TreeSelectOption; depth: number }> => {
  const result: Array<{ node: TreeSelectOption; depth: number }> = [];
...
export const filterTree = (
  ...
  const markMatches = (nodes: TreeSelectOption[]) => {
    for (const node of nodes) {
  ...
  const walk = (nodes: TreeSelectOption[]): TreeSelectOption[] => {
...
export const resolveValueTree = (
  value: NodeValue | NodeValue[] | null | undefined,
  index: NodeIndex
): TreeSelectOption[] => {
  if (value == null) return [];
  const values = Array.isArray(value) ? value : [value];
  return values.flatMap((v) => {
    const found = index.byValue.get(v);
    return found ? [found] : [];
  });
};

export const getDisplayTagValues = (
  value: NodeValue[],
  index: NodeIndex
): NodeValue[] => {
  const valueSet = new Set(value);
  return value.filter((v) => {
    const parent = index.parentOf.get(v);
    return parent == null || !valueSet.has(parent);
  });
};
